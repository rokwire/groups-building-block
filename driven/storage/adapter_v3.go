...
func (sa *Adapter) FindGroupsV3(context TransactionContext, OrgID string, filter model.GroupsFilter) ([]model.Group, error) {
	...
	groupFilter, memberships, err := sa.buildMainQuery(context, nil, OrgID, filter, true)
	if err != nil {
		return nil, err
	}
...
func (sa *Adapter) buildGroupsFilter11(OrgID string, context TransactionContext, filter model.GroupsFilter) (bson.D, error) {
	var groupIDs []string
...
func (sa *Adapter) CalculateGroupFilterStats(OrgID string, current *model.User, filter model.StatsFilter, skipMembershipCheck bool) (*model.StatsResult, error) {
	var result *model.StatsResult
	err := sa.PerformTransaction(func(ctx TransactionContext) error {
		baseFilter, _, err := sa.buildMainQuery(ctx, &current.ID, OrgID, filter.BaseFilter, skipMembershipCheck)
		if err != nil {
			return err
		}
		...
		for key, value := range filter.SubFilters {
			...
			filter, _, err := sa.buildMainQuery(ctx, &current.ID, OrgID, value, skipMembershipCheck)
			if err != nil {
				return err
			}
			if len(filter) > 0 {
