...
func (app *Application) getGroups(clientID string, current *model.User, filter model.GroupsFilter, isAdministrator bool) ([]model.Group, error) {
	var userID *string
	if current != nil {
		userID = &current.ID
	}
	// find the groups objects
	groups, err := app.storage.FindGroups(clientID, userID, filter, isAdministrator)
	if err != nil {
		return nil, err
	}
