...
func (auth *Auth1) idTokenCheck(w http.ResponseWriter, r *http.Request, allowAnonymousCoreToken bool) (string, *model.User) {
	...
	user := auth.idTokenAuth.check(OrgID, idToken, allowAnonymousCoreToken, nil, r)
	return OrgID, user
}
...
func (auth *Auth1) adminCheck(r *http.Request) (string, *model.User, bool) {
	...
	user, forbidden := auth.adminAuth.check(OrgID, r)
...
func (auth *IDTokenAuth) check(OrgID string, token *string, allowAnonymousCoreToken bool, allowedOrgIDs []string, r *http.Request) *model.User {
	var data *userData
...
func (auth *AdminAuth) check(OrgID string, r *http.Request) (*model.User, bool) {
	var data *userData
	var isCoreUser = false
	var coreErr error
	if auth.coreTokenAuth != nil {
